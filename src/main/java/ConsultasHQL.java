...
public class ConsultasHQL {
	...
	public static void main(String[] args) {
		...
		EmpleadoPorDni(
				"select empleado.dni, empleado.nom_emp, empleado_datos_prof.categoria, empleado_datos_prof.sueldo_bruto_anual from empleado_datos_prof join empleado on empleado_datos_prof.dni = empleado.dni WHERE empleado.dni like :dni",
				"35986421X");

		System.out.println("\n9. MOSTRAR NUMERO DE EMPLEADOS POR CADA DEPARTAMENTO");
		EmpleadoDepartamento(
				"select count(e.dni), e.departamento.nom_dpto from Empleado e inner join e.departamento group by e.departamento.nom_dpto");

		System.out.println("\n10. MOSTRAR NOMBRE Y DNI DE EMPLEADOS A PARTIR DE LA CATEGORIA");
		EmpleadoPorCategoria(
				"select e.empleado.nom_emp, e.empleado.dni, e.categoria from Empleado_datos_prof e inner join e.empleado where e.categoria like: categoria",
				"Grupo1");
		...
		/*System.out.println("\n13. BORRAR DEPARTAMENTO A PARTIR DEL NOMBRE");
		BorrarDepartamento("delete from departamento where departamento.nom_dpto = 'RRHH'");*/
	...
	public static void ListQueryDepartamentos(String consulta) {
		...
		for (Departamento departamento : listaDepartamentos) {
			System.out.println(
					"Departamento [ID: " + departamento.getId_dep() + "; Nombre: " + departamento.getNom_dpto() + "]");
		}
	}
	...
	public static void ListQueryEmpleados(String consulta) {
		...
		for (Object[] fila : objetos) {
			...
			System.out.println("Empleado [DNI: " + empleado.getDni() + "; Nombre: " + empleado.getNom_emp()
					+ "; Categoria: " + empleado_datos_prof.getCategoria() + "; Sueldo Bruto: "
					+ empleado_datos_prof.getSueldo_bruto_anual() + "]");
		}
	}
	...
	public static void ListQueryProyectos(String consulta) {
		...
		for (Proyecto proyecto : listaProyectos) {
			System.out.println("Proyecto [ID: " + proyecto.getId_proy() + "; Fecha Inicio: " + proyecto.getF_inicio()
					+ "; Fecha Fin: " + proyecto.getF_fin() + "; Nombre: " + proyecto.getNom_proy() + "]");
		}
	}
	...
	public static void EmpleadosSede(String consulta) {
		...
		for (Object[] fila : objetos) {
			...
			System.out.println("Numero de Empleados: " + numEmpleados + "; Numero de Proyectos: " + numProyectos
					+ "; Nombre de la Sede: " + nomSede);
		}
	}
	...
	public static void EmpleadoPorDni(String consulta, String dni) {
		...
		for (Object[] fila : objetos) {
			...
			System.out.println("Empleado [DNI: " + dni + "; Nombre: " + nombre + "; Categoria: " + categoria
					+ "; Sueldo: " + sueldo + "]");
		}
	}
	...
	public static void BorrarDepartamento(String consulta) {
		Transaction t = s.beginTransaction();
		Query query = s.createNativeQuery(consulta);
		int filasAfectadas = query.executeUpdate();
		t.commit();
